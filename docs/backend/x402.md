> **本文档来源**：pay-unify-backend 仓库的 `docs/X402_INTEGRATION_GUIDE.md`（保持与源码同步的权威版本）。
> 修改时请先更新仓库内原文件，再同步本页。

# SkillHub X402 支付集成指南

> ⚠️ 认证已迁移：本文部分示例仍为旧的 GoAuth(X-App-Id/X-Sign) 写法，已不适用；
> /api/v2/* 一律使用 OAuth2：`POST /oauth/token` 换取 `Authorization: Bearer <token>` 后调用。
...
| **pay-unify-backend** | 商户后端，处理微信支付下单、X402 签名、验证支付、执行业务 |
...
> 签名计算方式参考 `examples/python_payment_example.py` 中的 `_generate_sign()` 方法。
...
# 构建签名串（注意：SignIncludeBody = false）
SIGN_STR="AppId=${APP_ID}&Nonce=${NONCE}&Timestamp=${TIMESTAMP}"
SIGN=$(echo -n "$SIGN_STR" | openssl dgst -sha256 -hmac "$APP_SECRET" | awk '{print $2}')
